...
description: >
  ...
  (memory/facts/{anchor}.md) and a visual scene file (memory/scenes/{anchor}.md).
...
are written to the agent's git-backed memory directory:

  memory/facts/{anchor}.md   -- structured paraphrase, always written
  memory/scenes/{anchor}.md  -- visual scene description, written for FULL path
...
Write to: memory/facts/{anchor}.md
...
Frontmatter:
  ...
  linked_scene: memory/scenes/{anchor}.md    # omit for STREAMLINED path
...
Write to: memory/scenes/{anchor}.md
...
Frontmatter:
  ...
  type: scene
  linked_fact: memory/facts/{anchor}.md
  confidence: [HIGH | MEDIUM | LOW]
  stored: [YYYY-MM-DD]
  ---
...
After writing both files, commit to the agent's memory repo:
  ...
  git -C ~/.letta/agents/$(letta whoami --id)/memory add facts/ scenes/
  git -C ~/.letta/agents/$(letta whoami --id)/memory \
      commit -m "mem: store {anchor} ({type}, evidence:{score})"
  git -C ~/.letta/agents/$(letta whoami --id)/memory push
...
STATE A -- Fact file found AND scene file exists:
  1. Read memory/scenes/{anchor}.md
...
ENCODING DECISION FLOW:
  ...
  User shares information
        |
        v
  Check memory/facts/{anchor}.md -- exists? -- compare & resolve conflict
        |
        v (no duplicate)
...
FILE STRUCTURE (in agent memfs):
  ...
  memory/
  +-- system/
  |   +-- persona.md          <- always in context
  |   +-- human.md            <- always in context
  +-- facts/
...
FRONTMATTER (fact file):
  ...
  type: [discrete | compositional | relational]
  linked_scene: memory/scenes/{anchor}.md    # omit for streamlined
  confidence: [HIGH | MEDIUM | LOW]
  evidence_score: [0-12]
  stored: [YYYY-MM-DD]
  ---
